...
def test_reshape_layer_forward_prop():
    ...
    assert jnp.allclose(
        reshape.forward_prop(input_activations=Activations(X)),
        X.reshape(X.shape[0], 2, 2),
    )
...
def test_reshape_layer_backward_prop():
    ...
    result = reshape._backward_prop(dZ=cast(D[Activations], dZ))
    assert cast(jnp.ndarray, result).shape == (2, 4)
...
def test_flatten_layer_backward_prop():
    ...
    result = flatten.backward_prop(dZ=cast(D[Activations], dZ))
    assert jnp.allclose(cast(jnp.ndarray, result), jnp.arange(9).reshape((1, 3, 3)))
