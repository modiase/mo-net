...
class TestStreamingTrainingSet:
    """The 3-pass streaming pair builder."""

    def _corpus(self, tmp_path: Path) -> str:
        """A toy corpus diverse enough to survive stopword filter."""
    ...
    def test_returns_vocab_and_arrays(self, tmp_path: Path, monkeypatch):
        ...
        vocab, X, Y = build_streaming_training_set(
            corpus_url=url,
            limit=None,
            max_vocab_size=100,
            forced_words=(),
        ...
        assert len(vocab) >= 8  # plenty of distinct words
        assert X.shape[0] == Y.shape[0]
        assert X.shape[1] == 4  # 2 * context_size
        assert X.shape[0] > 0

    def test_cache_hit_on_second_call(self, tmp_path: Path, monkeypatch):
        from mo_net.samples.word2vec.vocab import build_streaming_training_set
        ...
        _, X1, Y1 = build_streaming_training_set(
            corpus_url=url,
            max_vocab_size=100,
            context_size=2,
            cache_dir=cache,
        ...
        # Second call should mmap the cached files (X.shape identical).
        _, X2, Y2 = build_streaming_training_set(
            corpus_url=url,
            max_vocab_size=100,
            context_size=2,
            cache_dir=cache,
    ...
    def test_skips_sentences_shorter_than_window(self, tmp_path: Path, monkeypatch):
        from mo_net.samples.word2vec.vocab import build_streaming_training_set
        ...
        _, X, Y = build_streaming_training_set(
            corpus_url=f"file://{path}",
            max_vocab_size=100,
            context_size=3,
            cache_dir=cache,
            subsample_t=0,
