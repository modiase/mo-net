...
def calculate(model_path: Path, expr: str, num_results: int):
    ...
    try:
        ...
        similarities = find_most_similar_words(result_vector, model, vocab, num_results)
        ...
        for word, similarity in similarities:
            click.echo(f"  {word}: {similarity:.4f}")

        compute_similarity(result_vector, model.embeddings[vocab[target_word]])
        click.echo()
        click.echo(f"Similarity to target word: {similarity:.4f}")
        ...
        for word in random_words:
            click.echo(
                f"  {word}: {compute_similarity(result_vector, model.embeddings[vocab[word]]):.4f}"
            )
